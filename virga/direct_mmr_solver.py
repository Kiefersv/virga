...
def direct_solver(temperature, pressure, condensibles, gas_mw, gas_mmr, rho_p, mw_atmos,
    ...
    for i, igas in zip(range(ngas), condensibles):
        ...
        qc, qt, rg, reff, ndz, dz, qc_path[i], mixl = calc_qc(z, P_z, T_z, T_P, kz,
                                                              gravity, gas_name, gas_mw[i], gas_mmr[i], rho_p[i],
                                                              mw_atmos, mh, fsed, sig, rmin, nrad,
    ...
    for i in range(len(qc_out)):
        ...
        if qc_out[i] == 0.0:  # layer is cloud free
            ...
            if analytical_rg:
                ...
                rg[i] = (fsed**(1./alpha) * rw[i] / fac_3pa_3)
                ...
                ndz[i] = (3 * rho_atmos(T, P) * qc_out[i] * dz[i] /
                            ( 4 * np.pi * rho_p * rg[i]**3 ) / fac_3)

            else:
                if size_distribution != 'lognormal':
                    print('[WARN] Non analytic solutions (analytic_rg=False) are '
                          'currently only supported with "lognormal" size distributions. '
                          'Consider setting analytic_rg=True.')
